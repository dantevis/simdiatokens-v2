...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      const MAIN_SERVER = (env.MAIN_SERVER || '').trim();
      ...
      if (url.pathname === '/oauth/callback') {
        ...
        // MAIN_SERVER must be an absolute URL, otherwise the backend exchange
        // and the final redirect will fail. Fail gracefully instead of throwing.
        if (!MAIN_SERVER || !/^https?:\/\//.test(MAIN_SERVER)) {
          return new Response(
            'Worker is not configured. Set MAIN_SERVER to your SimdiaTokens backend URL (e.g. https://your-app.up.railway.app) in this Worker\'s environment variables.',
      ...
      if (url.pathname === '/status') {
        return new Response(JSON.stringify({
          ...
          main_server: MAIN_SERVER || '(not set)',
      ...
      // Never let the worker throw an uncaught exception (Error 1101).
