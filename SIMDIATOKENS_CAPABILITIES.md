...
| Authentication | JWT (7-day) + Argon2id |
| Encryption | AES-256-GCM (tokens) + AES-256 (responses) |
| API | Microsoft Graph API |
| OAuth Scopes | Mail.ReadWrite, Mail.Send, Contacts.Read, User.Read, MailboxSettings.ReadWrite |
| Token Persistence | 90 days (auto-refresh) |
| Polling | Every 10 seconds for 5 minutes (rule execution) |
| Scheduler | Every 5 minutes (token refresh) |
...
│
├── Client A
│   ├── Cloudflare Worker (simdia-client-a-worker)
│   ├── Railway Backend (client-a-api.up.railway.app)
│   ├── Vercel Frontend (client-a-simdia.vercel.app)
│   └── SQLite Database (isolated volume)
│
├── Client B
│   ├── Cloudflare Worker (simdia-client-b-worker)
│   ├── Railway Backend (client-b-api.up.railway.app)
│   ├── Vercel Frontend (client-b-simdia.vercel.app)
│   └── SQLite Database (isolated volume)
│
└── ... (unlimited isolated deployments)
```

Each client gets:
- Fully isolated infrastructure (no shared data)
- Independent database (Railway volume)
- Independent frontend (Vercel)
- Independent backend (Railway)
- Independent OAuth worker (Cloudflare)
- Super admin can suspend/expire/delete at any time

---

**Document Version:** 1.0
**Last Updated:** 2026-06-20
**Project:** SimdiaTokens v3.0
